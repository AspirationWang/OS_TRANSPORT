...
static int validate_send_input(void *handle, urma_jetty_info_t *jetty_info,
                               ost_buffer_info_t *local_src, ost_buffer_info_t *remote_dst, uint32_t len,
...
static int validate_recv_input(void *handle, ost_buffer_info_t *host_src, ost_device_info_t *device_dst,
...
uint32_t common_split_chunks(uint64_t src_addr, uint64_t dst_addr, uint32_t len,
                             chunk_info_t **ret_chunks, uint64_t *ret_chunk_num)
{
...
// 发送数据时切分chunk
uint32_t send_split_chunks(ost_buffer_info_t *local_src, ost_buffer_info_t *remote_dst, uint32_t len,
                           chunk_info_t **ret_chunks, uint64_t *ret_chunk_num)
{
    uint64_t src_addr;
    uint64_t dst_addr;
...
// 接收数据时切分chunk
uint32_t recv_split_chunks(ost_buffer_info_t *host, ost_device_info_t *device, uint32_t len,
                           chunk_info_t **ret_chunks, uint64_t *ret_chunk_num)
{
    uint64_t src_addr;
    uint64_t dst_addr;
...
void construct_send_task_arg(send_task_arg_t *arg, urma_write_info_t write_info,
                             chunk_info_t *chunk_info, uint64_t chunk_id, bool is_last_chunk,
                             task_sync_t *sync)
{
    // 显式构造每个位域字段，避免隐式保留旧值
...
void construct_recv_task_arg(recv_task_arg_t *arg, urma_recv_info_t recv_info,
                             chunk_info_t *chunk_info, bool is_last_chunk, task_sync_t *sync)
{
...
static int register_send_tasks(os_transport_handle_t *ost_handle, chunk_info_t *chunks,
...
static int register_recv_tasks(os_transport_handle_t *ost_handle, chunk_info_t *chunks,
                               uint64_t chunk_num, int (*task_func)(void *), urma_info_t urma_info,
                               task_sync_t *sync)
{
    uint64_t *task_ids = NULL;
...
uint32_t construct_and_register_worker_task(os_transport_handle_t *ost_handle, chunk_info_t *chunks,
                                            uint64_t chunk_num, task_type_t type,
                                            int (*task_func)(void *), urma_info_t urma_info,
...
static int register_tasks_and_bind_chunks(os_transport_handle_t *ost_handle, chunk_info_t *chunks,
                                          uint64_t chunk_num, task_type_t type,
                                          int (*task_func)(void *), urma_info_t urma_info,
                                          task_sync_t **sync_handle)
{
...
static int send_single_chunk(urma_jetty_info_t *jetty_info, ost_buffer_info_t *local_src,
                             ost_buffer_info_t *remote_dst, uint32_t len, uint32_t server_key,
                             uint32_t client_key)
{
...
uint32_t wait_and_free_sync(void *handle, task_sync_t *sync_handle)
{
    ...
    if (!ost_handle || !sync_handle) {
        OST_LOG_ERROR(
            "Failed: invalid arguments (handle=%p, sync_handle=%p).", handle, (void *)sync_handle);
        return -1;
    }
